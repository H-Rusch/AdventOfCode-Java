# Advent of Code

Java & Kotlin solutions for the Advent of Code.

- [2015](https://adventofcode.com/2015)
- [2020](https://adventofcode.com/2020)
- [2025](https://adventofcode.com/2025)

## Input Handling

This project automatically downloads inputs when you execute the code.

Inputs are cached on disk after being downloaded once. This caching eliminates unnecessary requests to the Advent of Code website.

To enable this feature, ensure the following setup:

Set the `session` property in a `.env` file with your session value from the [Advent of Code](https://adventofcode.com) website. This is required for authenticating and downloading inputs.
