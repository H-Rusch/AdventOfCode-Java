...
class Day05 extends AbstractDay2015 {
  ...
  @NonNull
  @Override
  public Long part1(@NonNull String input) {
    return input.lines().filter(this::isNice).count();
  }

  @NonNull
  @Override
  public Long part2(@NonNull String input) {
    return input.lines().filter(this::isReallyNice).count();
  }

  /**
   * Nice string: - contains 3 vowels - at least one character that appears twice in a row - does
  ...
  /**
   * Really nice string: - contains a pair of two letter that appear at least twice in the string
   * without overlapping - contains at least one letter which repeats with exactly one letter in
   * between
   */
